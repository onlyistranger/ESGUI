...
static void _progbar_draw_base_v(Canvas *c, int x, int y, uint8_t w, uint16_t len)
{
    uint8_t half = w >> 1;
...
static void _progbar_draw_base_h(Canvas *c, int x, int y, uint8_t w, uint16_t len)
{
    uint8_t half = w >> 1;
...
void ESGUI_WidgetBmpFocusBox(Canvas *c, int x, int y, uint16_t w, uint16_t h) {
    ...
    int x_r = x + w + 1;
    int y_b = y + h + 1;
    int16_t len = (int16_t)(w + 4 - 10) / 2;
    if (len < 3) len = 3;
    if (len > (int16_t)(w + 4) / 2) len = (int16_t)(w + 4) / 2;
    int16_t h_f = (int16_t)(h + 4 - 10) / 2;
    if (h_f < 3) h_f = 3;
    if (h_f > (int16_t)(h + 4) / 2) h_f = (int16_t)(h + 4) / 2;
...
void ESGUI_WidgetBmpFocusBoxAnim(Canvas *c, int x, int y, uint16_t box_w, uint16_t box_h) {
    ...
    int16_t len = (int16_t)box_w / 4;
    if (len < 3) len = 3;
    if (len > (int16_t)(box_w / 2) - 2) len = (int16_t)(box_w / 2) - 2;
    int16_t h_f = (int16_t)box_h / 4;
    if (h_f < 3) h_f = 3;
    if (h_f > (int16_t)(box_h / 2) - 2) h_f = (int16_t)(box_h / 2) - 2;
