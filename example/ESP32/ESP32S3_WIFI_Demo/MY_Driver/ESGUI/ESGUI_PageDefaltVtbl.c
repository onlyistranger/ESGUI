...
void ESGUI_DefaultBoolPopWindowCreate(ESGUI_PopWindow_T *window,
    ...
    /* 恢复默认确定回调：bool_popwindow_items 为全局共享条目，调用方若覆盖过
     * on_enter（如首页 WiFi 开关弹窗），必须在此重置，否则后续所有布尔弹窗
     * 都会继承被覆盖的回调，导致绑定的布尔变量不更新 */
    bool_popwindow_items[0].on_enter = esgui_default_bool_popwindow_on_enter;
    bool_popwindow_items[1].on_enter = esgui_default_bool_popwindow_on_enter;
