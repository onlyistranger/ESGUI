 ...
 * @param  buf 显存缓冲区指针，不能为 ESGUI_NULL
 * @param  w   画布宽度（像素），必须大于 0
 * @param  h   画布高度（像素），必须大于 0 且建议为 8 的倍数
 * @note   初始化后裁剪栈默认压入全屏区域，clip_sp = 0
 */
void canvas_init(Canvas *c, eui_uint8_t *buf, int w, int h) {
    if (c == ESGUI_NULL || buf == ESGUI_NULL) return;
    if (w <= 0 || h <= 0) return;
 ...
 * @param  c  Canvas 指针，不能为 ESGUI_NULL
 * @param  a  待裁剪区域指针，不能为 ESGUI_NULL
...
bool canvas_clip_push(Canvas *c, const Area *a) {
    if (c == ESGUI_NULL || a == ESGUI_NULL) return false;
 ...
 * @param  draw_fn    用户绘制回调，不能为 ESGUI_NULL；接收 Canvas 指针和 user 参数
 * @param  user       绘制回调的用户数据
 * @param  flush_fn   送屏回调，不能为 ESGUI_NULL；参数为 (x1, y1, x2, y2, buf, flush_user)
...
void canvas_refresh_fullscreen(Canvas *c, int strip_h,
                                ...
                                void (*flush_fn)(int,int,int,int,const eui_uint8_t*,void*), void *flush_user)
{
    if (c == ESGUI_NULL || draw_fn == ESGUI_NULL || flush_fn == ESGUI_NULL) return;
 ...
 */
void canvas_clear(Canvas *c, eui_uint8_t color) {
    if (c == ESGUI_NULL) return;
...
void canvas_strip_iter_init(CanvasStripIter *it, Canvas *c, int strip_h) {
    if (it == ESGUI_NULL || c == ESGUI_NULL) return;
...
bool canvas_strip_iter_next(CanvasStripIter *it) {
    if (it == ESGUI_NULL || it->canvas == ESGUI_NULL) return false;
...
void canvas_strip_iter_bind(CanvasStripIter *it) {
    if (it == ESGUI_NULL || it->canvas == ESGUI_NULL) return;
...
void canvas_strip_iter_flush(CanvasStripIter *it,
    void (*flush_fn)(int,int,int,int,const eui_uint8_t*,void*), void *flush_user)
{
    if (it == ESGUI_NULL || flush_fn == ESGUI_NULL) return;
